...
namespace ShipWorks.Tests.Shipping.Insurance.InsureShip.Net.Claim
{
    public class InsureShipClaimStatusRequestTest : IDisposable
    {
        ...
        public void GetClaimStatus_DelegatesToWebClient()
        {
            ...
            var shipment = new ShipmentEntity { Processed = true, InsurancePolicy = policy };
            ...
            mock.Mock<IInsureShipWebClient>()
                .Verify(x => x.Submit<InsureShipGetClaimStatusResponse>("get_claim_status", It.Is<Dictionary<string, string>>(y => y["claim_id"] == "1234")));
        }
        ...
        public void GetClaimStatus_ReturnsStatus_WhenCallIsSuccessful()
        {
            ...
            var shipment = new ShipmentEntity { Processed = true, InsurancePolicy = policy };
            mock.Mock<IInsureShipWebClient>()
                .Setup(x => x.Submit<InsureShipGetClaimStatusResponse>(AnyString, It.IsAny<Dictionary<string, string>>()))
        ...
        {
            var shipment = new ShipmentEntity { Processed = true, InsurancePolicy = new InsurancePolicyEntity() };
            mock.Mock<IInsureShipWebClient>()
                .Setup(x => x.Submit<InsureShipGetClaimStatusResponse>(AnyString, It.IsAny<Dictionary<string, string>>()))
