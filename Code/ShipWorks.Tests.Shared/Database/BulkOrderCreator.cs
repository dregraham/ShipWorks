...
namespace ShipWorks.Tests.Shared.Database
{
    public class BulkOrderCreator
    {
        ...
        public IEnumerable<DateTime> OrderDates { get; set; }

        public IEnumerable<ShipmentTypeCode> ShipmentTypeCodes { get; set; }

        public IEnumerable<StoreTypeCode> StoreTypeCodes { get; set; }
        ...
        public void CreateOrderForAllStores()
        {
            using (ISqlAdapter sqlAdapter = new SqlAdapter())
            {
                ...
                foreach (StoreTypeCode storeTypeCode in StoreTypeCodes)
                {
                    ...
                    foreach (var orderDate in OrderDates)
                    {
                        ...
                        foreach (ShipmentTypeCode shipmentTypeCode in ShipmentTypeCodes)
                        {
                            ...
                            shipment.ProcessedDate = order.OrderDate;
                            ...
                            AddShippingLabelReference(shipment, sqlAdapter);

                            AddPrintResult(order.OrderID, shipment.ShipmentID, computer.ComputerID, sqlAdapter);
        ...
        private void AddEmailOutbound(long entityID, ISqlAdapter sqlAdapter)
        {
            ResourceEntity resource = new ResourceEntity();
            resource.InitializeNullsToDefault();
            resource.Data = new byte[] { 0 };
            resource.Filename = $"EmailResource_{Guid.NewGuid().ToString("N")}";
            ...
            ObjectReferenceEntity htmlObjectReference = new ObjectReferenceEntity()
            {
                ConsumerID = emailOutbound.EmailOutboundID,
                EntityID = emailOutbound.EmailOutboundID
            };
            sqlAdapter.SaveAndRefetch(htmlObjectReference);

            emailOutbound.HtmlPartResourceID = htmlObjectReference.ObjectReferenceID;
            sqlAdapter.SaveEntity(emailOutbound);



        }

        private void AddShippingLabelReference(ShipmentEntity shipment, ISqlAdapter sqlAdapter)
        {
            long shipmentOrPackageID = shipment.ShipmentID;

            if (shipment.ShipmentTypeCode == ShipmentTypeCode.DhlExpress)
            {
                shipmentOrPackageID = shipment.DhlExpress.Packages.First().DhlExpressPackageID;
            }
            else if (shipment.ShipmentTypeCode == ShipmentTypeCode.FedEx)
            {
                shipmentOrPackageID = shipment.FedEx.Packages.First().FedExPackageID;
            }
            else if (shipment.ShipmentTypeCode == ShipmentTypeCode.UpsOnLineTools || shipment.ShipmentTypeCode == ShipmentTypeCode.UpsWorldShip)
            {
                shipmentOrPackageID = shipment.Ups.Packages.First().UpsPackageID;
            }
            else if (shipment.ShipmentTypeCode == ShipmentTypeCode.iParcel)
            {
                shipmentOrPackageID = shipment.IParcel.Packages.First().IParcelPackageID;
            }

            ResourceEntity resource = new ResourceEntity();
            resource.InitializeNullsToDefault();
            resource.Data = new byte[] { 0 };
            resource.Filename = $"ShippingLabel_{Guid.NewGuid().ToString("N")}";
            resource.Checksum = CalculateChecksum(resource.Filename);
            sqlAdapter.SaveAndRefetch(resource);
            ...
            objectReference.ConsumerID = shipmentOrPackageID;
            sqlAdapter.SaveEntity(objectReference);
        }

        private void AddPrintResult(long orderID, long shipmentID, long computerID, ISqlAdapter sqlAdapter)
        {
            ResourceEntity resource = new ResourceEntity();
            resource.InitializeNullsToDefault();
            resource.Data = new byte[] { 0 };
            resource.Filename = $"PrintResult_{Guid.NewGuid().ToString("N")}";
            resource.Checksum = CalculateChecksum(resource.Filename);
            sqlAdapter.SaveAndRefetch(resource);
            ...
            objectReference.EntityID = resource.ResourceID;
            objectReference.ReferenceKey = resource.Filename;
            sqlAdapter.SaveAndRefetch(objectReference);

            PrintResultEntity printResult = new PrintResultEntity();
            printResult.InitializeNullsToDefault();
            printResult.ComputerID = computerID;
            printResult.RelatedObjectID = orderID;
            printResult.ContextObjectID = shipmentID;
            printResult.ContentResourceID = objectReference.ObjectReferenceID;
            printResult.JobIdentifier = Guid.NewGuid();
            sqlAdapter.SaveAndRefetch(printResult);

            objectReference.ConsumerID = printResult.PrintResultID;
            sqlAdapter.SaveAndRefetch(objectReference);
