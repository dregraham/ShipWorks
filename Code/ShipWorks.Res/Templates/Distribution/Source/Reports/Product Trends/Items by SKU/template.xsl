<!DOCTYPE xsl:stylesheet [
    ...
    <!ENTITY itemKey "SKU($optionSpecific)">
...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sw="http://www.interapptive.com/shipworks" extension-element-prefixes="sw">
   ...
   <xsl:key name="items-non-specific" match="Item" use="SKU" />
    ...
    <body style="{$pageFont}">
       <!-- Determine the order with the larget order number -->
        ...
        <h3>Items by SKU - <xsl:value-of select="count(Customer/Order)" /> Orders</h3>
        ...
        <table style="width:{$pageWidth}; margin: 0px 0px -6px 0px; border-collapse: collapse;" cellspacing="0">
            ...
            <!-- Group by item -->
             <xsl:for-each select="Customer/Order/Item[generate-id(.)=generate-id(key($keyTable, SKU))]">
                <xsl:sort select="SKU" order="ascending" />
