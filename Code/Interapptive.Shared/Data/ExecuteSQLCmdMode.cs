...
namespace Interapptive.Shared.Data
{
    ...
    public class ExecuteSQLCmdMode
    {
        ...
        private readonly SqlConnection sqlConn; // Hidden
        ...
        public ExecuteSQLCmdMode(SqlConnection sqlConn)
        {
            ...
            variables.Add("SQLCMDSERVER", sqlConn.DataSource);
            variables.Add("SQLCMDWORKSTATION", sqlConn.WorkstationId);
            variables.Add("SQLCMDDBNAME", sqlConn.Database);
            variables.Add("SQLCMDLOGINTIMEOUT", sqlConn.ConnectionTimeout.ToString(CultureInfo.InvariantCulture));
            ...
            for (int i = 0; i < SQLScript.GetUpperBound(0); i++)
            {
                ...
                if (!ignoredCommands.Contains((ucaseLine).Split(' ')[0]))
                {
                    ...
                    if (ucaseLine.StartsWith("GO "))
                    {
                        try
                        {
                            if (sqlCmd.Length > 0)
                            {
                                ...
                                var sqlComm = new SqlCommand(sqlCmd.ToString(), sqlConn);
                        ...
                        catch (Exception ex)
                        {
                            if (exitOnError)
                            {
                                throw new Exception("Error executing " + sqlCmd, ex);
                            }

                            exceptions.Add(new Exception("Error executing " + sqlCmd, ex));
                        }
                    ...
                    else
                    {
                        ...
                        int begPos;
                        string temp;
                        if (ucaseLine.StartsWith(":SETVAR "))
                        {
                        ...
                        else
                        {
                            ...
                            if (temp.Length > 4 && temp.Contains("$("))
                            {
                                ...
                                foreach (var keyPair in variables)
                                {
                                    ...
                                    begPos = temp.ToUpper().IndexOf(SearchFor, StringComparison.Ordinal);
                                    while (begPos >= 0)
                                    {
                                        ...
                                        begPos = temp.ToUpper().IndexOf(string.Format(SearchFor), StringComparison.Ordinal);
                                    }
                                }
                            }
