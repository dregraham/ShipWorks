...
namespace Interapptive.Shared.IO.Text.HtmlAgilityPack
{
	...
	public class HtmlNode: IXPathNavigable
	{
		...
		internal HtmlAgilityDocument _ownerdocument;
		...
		public static HtmlNode CreateNode(string html)
		{
			...
			HtmlAgilityDocument doc = new HtmlAgilityDocument();
			doc.LoadHtml(html);
		...
		internal HtmlNode(HtmlNodeType type, HtmlAgilityDocument ownerdocument, int index)
		{
			_nodetype = type;
		...
		public string Id
		{
			get
			{
				...
				{
					throw new Exception(HtmlAgilityDocument.HtmlExceptionUseIdAttributeFalse);
				}
				return GetId();
			}
			set
		...
		public HtmlNode RemoveChild(HtmlNode oldChild)
		{
			...
			if (index == -1)
			{
				throw new ArgumentException(HtmlAgilityDocument.HtmlExceptionRefNotChild);
			}
		...
		/// </summary>
		public HtmlAgilityDocument OwnerDocument
		{
			get
			{
		...
		internal void WriteAttribute(TextWriter outText, HtmlAttribute att)
		{
			...
			if (_ownerdocument.OptionOutputAsXml)
			{
				...
				outText.Write(" " + name + "=\"" + HtmlAgilityDocument.HtmlEncode(att.XmlValue) + "\"");
			}
			else
			{
				if (_ownerdocument.OptionOutputUpperCase)
		...
		public void WriteTo(TextWriter outText)
		{
			...
			switch(_nodetype)
			{
				...
				case HtmlNodeType.Text:
					...
					if (_ownerdocument.OptionOutputAsXml)
					{
						outText.Write(HtmlAgilityDocument.HtmlEncode(html));
					}
					else
					{
						outText.Write(html);
				...
				case HtmlNodeType.Element:
					...
					if (_ownerdocument.OptionOutputAsXml)
					{
						if (name.Length > 0)
						{
							...
							name = HtmlAgilityDocument.GetXmlName(name);
						}
						else
						{
							break;
