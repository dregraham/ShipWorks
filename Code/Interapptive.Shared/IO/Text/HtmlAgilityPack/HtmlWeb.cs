...
namespace Interapptive.Shared.IO.Text.HtmlAgilityPack
{
	...
	public class HtmlWeb
	{
		...
		/// </summary>
		public delegate void PreHandleDocumentHandler(HtmlAgilityDocument document);
		...
		public HtmlAgilityDocument Load(string url)
		{
			return Load(url, "GET");
		}
		...
		/// <param name="method">The HTTP method used to open the connection, such as GET, POST, PUT, or PROPFIND.</param>
		/// <returns>A new HTML document.</returns>
		public HtmlAgilityDocument Load(string url, string method)
		{
			Uri uri = new Uri(url);
			HtmlAgilityDocument doc;
			if ((uri.Scheme == Uri.UriSchemeHttps) ||
				(uri.Scheme == Uri.UriSchemeHttp))
			{
		...
		private HtmlAgilityDocument LoadUrl(Uri uri, string method)
		{
		...
		private HttpStatusCode Get(Uri uri, string method, string path, HtmlAgilityDocument doc)
		{
...
//				}
		...
		{
			HtmlAgilityDocument doc = Load(htmlUrl);
			doc.Save(writer);
		}
