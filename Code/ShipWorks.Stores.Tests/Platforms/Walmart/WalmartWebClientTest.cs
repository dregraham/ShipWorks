...
namespace ShipWorks.Stores.Tests.Platforms.Walmart
{
    public class WalmartWebClientTest : IDisposable
    {
        ...
        public WalmartWebClientTest()
        {
            ...
            mock2 = AutoMockExtensions.GetLooseThatReturnsMocks();
        }
        ...
        public void GetOrders_AsknowledgesEachOrder()
        {
            ...
            mock.Mock<IHttpRequestSubmitterFactory>()
                .Setup(f => f.GetHttpVariableRequestSubmitter())
                .Returns(requestSubmitter);

            Mock<IHttpVariableRequestSubmitter> acknowledgeRequestSubmitter = mock.Mock<IHttpVariableRequestSubmitter>();
            requestSubmitter.Setup(r => r.GetResponse()).Returns(responseReader);

            mock.Mock<IHttpRequestSubmitterFactory>()
                .Setup(f => f.GetHttpTextPostRequestSubmitter(It.IsAny<string>(), It.IsAny<string>()))
                .Returns(acknowledgeRequestSubmitter.Object);
