    ...
    public partial class AddressViewModel : IOrderLookupFieldLayoutProviderHost, INotifyPropertyChanged, INotifyPropertyChanging, IDisposable
    {
        ...
        private async void ValidateAddress()
        {
            ...
            using (messageHelper.ShowProgressDialog("Validating Addresses", "ShipWorks is validating the addresses."))
            {
                ...
                bool canAdjustAddress = false;
                if(addressAdapter.CountryCode == "US" && store.DomesticAddressValidationSetting == AddressValidationStoreSettingType.ValidateAndApply)
                {
                    canAdjustAddress = true;
                }
                else if(addressAdapter.CountryCode != "US" && store.InternationalAddressValidationSetting == AddressValidationStoreSettingType.ValidateAndApply)
                {
                    canAdjustAddress = true;
                }
