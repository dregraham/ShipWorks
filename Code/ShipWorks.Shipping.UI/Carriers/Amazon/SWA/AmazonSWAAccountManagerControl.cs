...
namespace ShipWorks.Shipping.UI.Carriers.Amazon.SWA
{
    ...
    public partial class AmazonSWAAccountManagerControl : UserControl
    {
        ...
        /// Constructor
        /// </summary>
        public AmazonSWAAccountManagerControl()
        {
            InitializeComponent();
        }
        ...
        private void UpdateButtonState()
        {
            ...
            // add should only be enabled if there are no accounts
            add.Enabled = repo.AccountsReadOnly.None();
            using (ILifetimeScope scope = IoC.BeginLifetimeScope())
            {
        ...
        private async void OnDelete(object sender, EventArgs e)
        {
            ...
            IAmazonSWAAccountEntity shipper = (IAmazonSWAAccountEntity) row.Tag;
            ...
            if (result == DialogResult.OK)
            {
                using (ILifetimeScope scope = IoC.BeginLifetimeScope())
                {
                    Result disconnectResult = await scope.Resolve<IShipEngineWebClient>().DisconnectAmazonShippingAccount(shipper.ShipEngineCarrierId);
                    if (disconnectResult.Failure)
                    {
                        MessageHelper.ShowError(this, $"An error occurred deleting account {disconnectResult.Message}");
                    }
                }
