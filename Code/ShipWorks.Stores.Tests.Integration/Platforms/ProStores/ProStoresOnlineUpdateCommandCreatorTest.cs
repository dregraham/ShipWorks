...
namespace ShipWorks.Stores.Tests.Integration.Platforms.ProStores
{
    ...
    public partial class ProStoresOnlineUpdateCommandCreatorTest : IDisposable
    {
        ...
        public ProStoresOnlineUpdateCommandCreatorTest(DatabaseFixture db, ITestOutputHelper output)
        {
            ...
            {
                webClient = mock.Override<IProStoresWebClient>();
        ...
        public async Task UploadDetails_MakesOneWebRequest_WhenOrderIsNotCombined()
        {
            ...
            await commandCreator.OnUploadDetails(menuContext.Object);
        ...
        public async Task UploadDetails_ContinuesUploading_WhenFailuresOccur()
        {
            ...
            webClient.Setup(x => x.UploadShipmentDetails(store, AnyShipment, DetailsWithID(10)))
                .Throws(new ProStoresException("Foo"));
        ...
        /// </summary>
        public static OrderUploadDetails DetailsWithID(long id) =>
            It.Is<OrderUploadDetails>(o => o.OrderNumber == id);
        ...
        private OrderEntity CreateNormalOrder(int orderRoot, string trackingNumber, bool manual, params int[] roots)
        {
            var order = roots.Aggregate(
                Create.Order<ProStoresOrderEntity>(store, context.Customer),
                (o, root) => AddItem(o, root, false))
                .Set(x => x.ConfirmationNumber, (orderRoot * 10000).ToString())
        ...
        {
            var order = Create.Order<ProStoresOrderEntity>(store, context.Customer)
                .Set(x => x.ConfirmationNumber, (orderRoot * 10000).ToString())
                .Set(x => x.OrderNumber, orderRoot * 10)
            ...
            foreach (var details in combinedOrderDetails)
            {
                ...
                if (!manual)
                {
                    Create.Entity<ProStoresOrderSearchEntity>()
                        .Set(x => x.OrderID, order.OrderID)
                        .Set(x => x.OriginalOrderID, idRoot * -1006)
                        .Set(x => x.ConfirmationNumber, (idRoot * 10000).ToString())
                        .Save();
                }
            }
        ...
        /// </summary>
        private OrderEntityBuilder<ProStoresOrderEntity> AddItem(OrderEntityBuilder<ProStoresOrderEntity> builder, int root, bool setOriginalOrderID) =>
