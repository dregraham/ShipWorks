...
namespace ShipWorks.Stores.Tests.Integration.Platforms.ProStores
{
    ...
    public class ProStoresCombineAndSplitTest : IDisposable
    {
        ...
        /// </summary>
        private async Task<(ProStoresOrderEntity original, ProStoresOrderEntity split)> PerformSplit(OrderEntity order)
        {
            ...
            var original = await dataProvider.GetEntityAsync<ProStoresOrderEntity>(result.First());
            var split = await dataProvider.GetEntityAsync<ProStoresOrderEntity>(result.Last());
