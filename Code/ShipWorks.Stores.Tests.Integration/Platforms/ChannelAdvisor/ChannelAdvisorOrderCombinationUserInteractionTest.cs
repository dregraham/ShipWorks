...
namespace ShipWorks.Stores.Tests.Integration.Platforms.ChannelAdvisor
{
    ...
    public class OrderCombinationUserInteraction : IDisposable
    {
        ...
        public OrderCombinationUserInteraction(DatabaseFixture db, ITestOutputHelper output)
        {
            ...
            store = Create.Store<ChannelAdvisorStoreEntity>(StoreTypeCode.ChannelAdvisor)
                .Save();

            // Create a dummy order that serves as a guarantee that we're not just fetching all orders later
            Create.Order<ChannelAdvisorOrderEntity>(store, context.Customer).Save();

            var order1 = Create.Order<ChannelAdvisorOrderEntity>(store, context.Customer)
                .Set(x => x.CustomOrderIdentifier, "1000")
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "100")
                .Save();
            var order2 = Create.Order<ChannelAdvisorOrderEntity>(store, context.Customer)
                .Set(x => x.CustomOrderIdentifier, "2000")
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "200")
                .Save();
            var order3 = Create.Order<ChannelAdvisorOrderEntity>(store, context.Customer)
        ...
        public async Task VariousOrderCombinations(bool[] manualOrders, int firstSurviving, int secondSurviving)
        {
            ...
            var storeQuery = factory.ChannelAdvisorOrderSearch.Where(ChannelAdvisorOrderSearchFields.OrderID == finalOrder.OrderID);
            ...
            using (ISqlAdapter sqlAdapter = context.Mock.Container.Resolve<ISqlAdapterFactory>().Create())
            {
                ...
                var orderTests = orders.Take(3).Select(x => x.Value)
                    ...
                    .LeftJoin(storeResults.OfType<IChannelAdvisorOrderSearchEntity>(), x => x.Item1.OrderID, x => x.OriginalOrderID)
                ...
                for (int i = 0; i < orderTests.Length; i++)
                {
                    ...
                    if (manualOrders[i])
                    {
                        Assert.Null(test.StoreSearch as IChannelAdvisorOrderSearchEntity);
                    }
                    else
                    {
                        Assert.Equal((test.Order as IChannelAdvisorOrderEntity).CustomOrderIdentifier,
                            (test.StoreSearch as IChannelAdvisorOrderSearchEntity).CustomOrderIdentifier);
                    }
                }
            }
        }
