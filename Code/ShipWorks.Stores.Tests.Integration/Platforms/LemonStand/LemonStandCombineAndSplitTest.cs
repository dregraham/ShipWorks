...
namespace ShipWorks.Stores.Tests.Integration.Platforms.LemonStand.OnlineUpdating
{
    [Collection("Database collection")]
    [Trait("Category", "ContinuousIntegration")]
    [Trait("Category", "CombineSplit")]
    public class LemonStandCombineAndSplitTest : IDisposable
    {
        ...
        public LemonStandCombineAndSplitTest(DatabaseFixture db, ITestOutputHelper output)
        {
            this.output = output;
            ...
            orderA = Create.Order<LemonStandOrderEntity>(store, context.Customer)
                .WithItem<LemonStandOrderItemEntity>(i => i.Set(x => x.ShortDescription, "10"))
                .Set(x => x.LemonStandOrderID, "10")
            ...
            orderB = Create.Order<LemonStandOrderEntity>(store, context.Customer)
                .WithItem<LemonStandOrderItemEntity>(i => i.Set(x => x.ShortDescription, "20"))
                .Set(x => x.LemonStandOrderID, "20")
            ...
            orderD = Create.Order<LemonStandOrderEntity>(store, context.Customer)
                .WithItem<LemonStandOrderItemEntity>(i => i.Set(x => x.ShortDescription, "30"))
                .Set(x => x.LemonStandOrderID, "30")
        ...
        public async Task Split_WithOrderNumbers()
        {
            ...
            var identityProvider = context.Mock.Container.Resolve<LemonStandCombineOrderIdSearchProvider>();
            ...
            Assert.Equal(new[] { "10" }, identities_A_0);
            Assert.Equal(new[] { "10" }, identities_A_1);
            Assert.Equal(new[] { "10" }, identities_A_2);
            Assert.Equal(new[] { "10" }, identities_A_3);
            Assert.Equal(new[] { "10" }, identities_A_4);
            Assert.Equal(new[] { "10" }, identities_A_5);
        }
        ...
        public async Task CombineSplitCombine_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { "10", "20" }, identities_A_C_1);
            Assert.Equal(new[] { "10", "20", "30" }, identities_D_C);
        }
        ...
        public async Task SplitManualCombineWithNotManualOrder_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { "20" }, identities_A_M_1_C);
        ...
        /// </summary>
        private async Task<(LemonStandOrderEntity original, LemonStandOrderEntity split)> PerformSplit(OrderEntity order)
        {
            ...
            var original = await dataProvider.GetEntityAsync<LemonStandOrderEntity>(result.First());
            var split = await dataProvider.GetEntityAsync<LemonStandOrderEntity>(result.Last());
