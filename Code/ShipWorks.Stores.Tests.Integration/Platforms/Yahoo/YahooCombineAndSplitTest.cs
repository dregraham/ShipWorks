...
namespace ShipWorks.Stores.Tests.Integration.Platforms.Yahoo
{
    ...
    public class YahooCombineAndSplitTest : IDisposable
    {
        ...
        public YahooCombineAndSplitTest(DatabaseFixture db, ITestOutputHelper output)
        {
            this.output = output;
            ...
            orderA = Create.Order<YahooOrderEntity>(store, context.Customer)
                .Set(x => x.YahooOrderID, "1000")
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "10")
                .Save();
            orderB = Create.Order<YahooOrderEntity>(store, context.Customer)
                .Set(x => x.YahooOrderID, "2000")
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "20")
                .Save();
            orderD = Create.Order<YahooOrderEntity>(store, context.Customer)
                .Set(x => x.YahooOrderID, "3000")
        ...
        public async Task Split_WithOrderNumbers()
        {
            ...
            var identityProvider = context.Mock.Container.Resolve<IYahooCombineOrderSearchProvider>();
        ...
        /// </summary>
        private async Task<(YahooOrderEntity original, YahooOrderEntity split)> PerformSplit(OrderEntity order)
        {
            ...
            var original = await dataProvider.GetEntityAsync<YahooOrderEntity>(result.First());
            var split = await dataProvider.GetEntityAsync<YahooOrderEntity>(result.Last());
