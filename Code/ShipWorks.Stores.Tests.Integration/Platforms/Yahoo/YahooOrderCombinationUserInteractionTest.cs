...
namespace ShipWorks.Stores.Tests.Integration.Platforms.Yahoo
{
    ...
    public class YahooOrderCombinationUserInteraction : IDisposable
    {
        ...
        public YahooOrderCombinationUserInteraction(DatabaseFixture db, ITestOutputHelper output)
        {
            this.output = output;
            ...
            store = Create.Store<YahooStoreEntity>(StoreTypeCode.Yahoo)
                .Save();

            // Create a dummy order that serves as a guarantee that we're not just fetching all orders later
            Create.Order<YahooOrderEntity>(store, context.Customer).Save();

            var order1 = Create.Order<YahooOrderEntity>(store, context.Customer)
                .Set(x => x.YahooOrderID, "1000")
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "100")
                .Save();
            var order2 = Create.Order<YahooOrderEntity>(store, context.Customer)
                .Set(x => x.YahooOrderID, "2000")
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "200")
                .Save();
            var order3 = Create.Order<YahooOrderEntity>(store, context.Customer)
        ...
        public async Task VariousOrderCombinations(bool[] manualOrders, int firstSurviving, int secondSurviving)
        {
            ...
            var storeQuery = factory.YahooOrderSearch.Where(YahooOrderSearchFields.OrderID == finalOrder.OrderID);
            ...
            using (ISqlAdapter sqlAdapter = context.Mock.Container.Resolve<ISqlAdapterFactory>().Create())
            {
                ...
                var orderTests = orders.Take(3).Select(x => x.Value)
                    ...
                    .LeftJoin(storeResults.OfType<IYahooOrderSearchEntity>(), x => x.Item1.OrderID, x => x.OriginalOrderID)
                ...
                for (int i = 0; i < orderTests.Length; i++)
                {
                    ...
                    if (manualOrders[i])
                    {
                        Assert.Null(test.StoreSearch as IYahooOrderSearchEntity);
                    }
                    else
                    {
                        Assert.Equal((test.Order as IYahooOrderEntity).YahooOrderID,
                            (test.StoreSearch as IYahooOrderSearchEntity).YahooOrderID);
                    }
                }
            }
        }
