...
namespace ShipWorks.Stores.Tests.Integration.Platforms.Magento
{
    ...
    public class MagentoOrderCombinationUserInteraction : IDisposable
    {
        ...
        public MagentoOrderCombinationUserInteraction(DatabaseFixture db, ITestOutputHelper output)
        {
            this.output = output;
            ...
            Create.Order<MagentoOrderEntity>(store, context.Customer).Save();
        ...
        public async Task VariousOrderCombinations(bool[] manualOrders, int firstSurviving, int secondSurviving)
        {
            ...
            var storeQuery = factory.MagentoOrderSearch.Where(MagentoOrderSearchFields.OrderID == finalOrder.OrderID);
            ...
            using (ISqlAdapter sqlAdapter = context.Mock.Container.Resolve<ISqlAdapterFactory>().Create())
            {
                ...
                var orderTests = orders.Take(3).Select(x => x.Value)
                    ...
                    .LeftJoin(storeResults.OfType<IMagentoOrderSearchEntity>(), x => x.Item1.OrderID, x => x.OriginalOrderID)
                ...
                for (int i = 0; i < orderTests.Length; i++)
                {
                    ...
                    if (manualOrders[i])
                    {
                        Assert.Null(test.StoreSearch as IMagentoOrderSearchEntity);
                    }
                    else
                    {
                        Assert.Equal((test.Order as IMagentoOrderEntity).MagentoOrderID,
                            (test.StoreSearch as IMagentoOrderSearchEntity).MagentoOrderID);
                    }
                }
            }
        }
