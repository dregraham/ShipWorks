...
namespace ShipWorks.Stores.Tests.Integration.Platforms.Jet
{
    ...
    public class OrderCombinationUserInteraction : IDisposable
    {
        ...
        public OrderCombinationUserInteraction(DatabaseFixture db, ITestOutputHelper output)
        {
            ...
            Create.Order<JetOrderEntity>(store, context.Customer).Save();

            var order1 = Create.Order<JetOrderEntity>(store, context.Customer)
                .Set(x => x.MerchantOrderId, "1000")
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "100")
                .Save();
            var order2 = Create.Order<JetOrderEntity>(store, context.Customer)
                .Set(x => x.MerchantOrderId, "2000")
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "200")
                .Save();
            var order3 = Create.Order<JetOrderEntity>(store, context.Customer)
        ...
        public async Task VariousOrderCombinations(bool[] manualOrders, int firstSurviving, int secondSurviving)
        {
            ...
            var storeQuery = factory.JetOrderSearch.Where(JetOrderSearchFields.OrderID == finalOrder.OrderID);
            ...
            using (ISqlAdapter sqlAdapter = context.Mock.Container.Resolve<ISqlAdapterFactory>().Create())
            {
                ...
                var orderTests = orders.Take(3).Select(x => x.Value)
                    ...
                    .LeftJoin(storeResults.OfType<IJetOrderSearchEntity>(), x => x.Item1.OrderID, x => x.OriginalOrderID)
                ...
                for (int i = 0; i < orderTests.Length; i++)
                {
                    ...
                    if (manualOrders[i])
                    {
                        Assert.Null(test.StoreSearch as IJetOrderSearchEntity);
                    }
                    else
                    {
                        Assert.Equal((test.Order as IJetOrderEntity).MerchantOrderId,
                            (test.StoreSearch as IJetOrderSearchEntity).MerchantOrderID);
                    }
                }
            }
        }
