...
namespace ShipWorks.Stores.Tests.Integration.Platforms.Jet
{
    ...
    public class JetCombineAndSplitTest : IDisposable
    {
        ...
        public JetCombineAndSplitTest(DatabaseFixture db, ITestOutputHelper output)
        {
            this.output = output;
            ...
            store = Create.Store<JetStoreEntity>()
                .Set(x => x.StoreTypeCode, StoreTypeCode.Jet)
                .Save();
            ...
            orderA = Create.Order<JetOrderEntity>(store, context.Customer)
                .Set(x => x.MerchantOrderId, "1000")
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "10")
                .Save();
            orderB = Create.Order<JetOrderEntity>(store, context.Customer)
                .Set(x => x.MerchantOrderId, "2000")
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "20")
                .Save();
            orderD = Create.Order<JetOrderEntity>(store, context.Customer)
                .Set(x => x.MerchantOrderId, "3000")
        ...
        public async Task Split_WithOrderNumbers()
        {
            ...
            var identityProvider = context.Mock.Container.Resolve<IJetOrderSearchProvider>();
        ...
        /// </summary>
        private async Task<(JetOrderEntity original, JetOrderEntity split)> PerformSplit(OrderEntity order)
        {
            ...
            var original = await dataProvider.GetEntityAsync<JetOrderEntity>(result.First());
            var split = await dataProvider.GetEntityAsync<JetOrderEntity>(result.Last());
