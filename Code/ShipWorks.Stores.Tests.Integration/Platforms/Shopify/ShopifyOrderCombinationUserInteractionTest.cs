...
namespace ShipWorks.Stores.Tests.Integration.Platforms.Shopify
{
    ...
    public class OrderCombinationUserInteraction : IDisposable
    {
        ...
        public OrderCombinationUserInteraction(DatabaseFixture db, ITestOutputHelper output)
        {
            ...
            store = Create.Store<ShopifyStoreEntity>(StoreTypeCode.Shopify)
                .Save();

            // Create a dummy order that serves as a guarantee that we're not just fetching all orders later
            Create.Order<ShopifyOrderEntity>(store, context.Customer).Save();

            var order1 = Create.Order<ShopifyOrderEntity>(store, context.Customer)
                .Set(x => x.ShopifyOrderID, 1000)
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "100")
                .Save();
            var order2 = Create.Order<ShopifyOrderEntity>(store, context.Customer)
                .Set(x => x.ShopifyOrderID, 2000)
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "200")
                .Save();
            var order3 = Create.Order<ShopifyOrderEntity>(store, context.Customer)
                .Set(x => x.ShopifyOrderID, 3000)
        ...
        public async Task VariousOrderCombinations(bool[] manualOrders, int firstSurviving, int secondSurviving)
        {
            ...
            var storeQuery = factory.ShopifyOrderSearch.Where(ShopifyOrderSearchFields.OrderID == finalOrder.OrderID);
            ...
            using (ISqlAdapter sqlAdapter = context.Mock.Container.Resolve<ISqlAdapterFactory>().Create())
            {
                ...
                var orderTests = orders.Take(3).Select(x => x.Value)
                    ...
                    .LeftJoin(storeResults.OfType<IShopifyOrderSearchEntity>(), x => x.Item1.OrderID, x => x.OriginalOrderID)
                ...
                for (int i = 0; i < orderTests.Length; i++)
                {
                    ...
                    if (manualOrders[i])
                    {
                        Assert.Null(test.StoreSearch as IShopifyOrderSearchEntity);
                    }
                    else
                    {
                        Assert.Equal((test.Order as IShopifyOrderEntity).ShopifyOrderID,
                            (test.StoreSearch as IShopifyOrderSearchEntity).ShopifyOrderID);
                    }
                }
            }
        }
