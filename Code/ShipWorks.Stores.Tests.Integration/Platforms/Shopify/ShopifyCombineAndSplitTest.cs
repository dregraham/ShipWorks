...
namespace ShipWorks.Stores.Tests.Integration.Platforms.Shopify
{
    ...
    public class ShopifyCombineAndSplitTest : IDisposable
    {
        ...
        public ShopifyCombineAndSplitTest(DatabaseFixture db, ITestOutputHelper output)
        {
            this.output = output;
            ...
            store = Create.Store<ShopifyStoreEntity>()
            ...
            orderA = Create.Order<ShopifyOrderEntity>(store, context.Customer)
                .Set(x => x.ShopifyOrderID, 1000L)
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "10")
                .Save();
            orderB = Create.Order<ShopifyOrderEntity>(store, context.Customer)
                .Set(x => x.ShopifyOrderID, 2000L)
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "20")
                .Save();
            orderD = Create.Order<ShopifyOrderEntity>(store, context.Customer)
                .Set(x => x.ShopifyOrderID, 3000L)
        ...
        public async Task Split_WithOrderNumbers()
        {
            ...
            var identityProvider = context.Mock.Container.Resolve<IShopifyOrderSearchProvider>();
            ...
            Assert.True(identities_A_0.All(i => i == 1000L));
            Assert.True(identities_A_1.All(i => i == 1000L));
            Assert.True(identities_A_2.All(i => i == 1000L));
            Assert.True(identities_A_3.All(i => i == 1000L));
            Assert.True(identities_A_4.All(i => i == 1000L));
            Assert.True(identities_A_5.All(i => i == 1000L));
        }
        ...
        public async Task CombineSplitCombine_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 1000L, 2000L }, identities_A_C_1);
            Assert.Equal(new[] { 1000L, 2000L, 3000L }, identities_D_C);
        }
        ...
        public async Task SplitThenCombineOrder_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 1000L, 2000L }, identities_A_C);
        ...
        public async Task CombineSplitWithBSurviving_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 1000L, 2000L }, identities_B_0);
            Assert.Equal(new[] { 1000L, 2000L }, identities_B_1);
        }
        ...
        public async Task CombineSplitWithASurviving_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 1000L, 2000L }, identities_A_0);
            Assert.Equal(new[] { 1000L, 2000L }, identities_A_1);
        }
        ...
        public async Task SplitACombineBCombineRemainingTwo_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 1000L, 2000L }, identities_A_1_C_1);
        }
        ...
        /// </summary>
        private async Task<(ShopifyOrderEntity original, ShopifyOrderEntity split)> PerformSplit(OrderEntity order)
        {
            ...
            var original = await dataProvider.GetEntityAsync<ShopifyOrderEntity>(result.First());
            var split = await dataProvider.GetEntityAsync<ShopifyOrderEntity>(result.Last());
