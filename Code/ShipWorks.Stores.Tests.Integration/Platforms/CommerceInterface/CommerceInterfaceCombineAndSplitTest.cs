...
namespace ShipWorks.Stores.Tests.Integration.Platforms.CommerceInterface
{
    ...
    public class CommerceInterfaceCombineAndSplitTest : IDisposable
    {
        ...
        public CommerceInterfaceCombineAndSplitTest(DatabaseFixture db, ITestOutputHelper output)
        {
            this.output = output;
            ...
            store = Create.Store<GenericModuleStoreEntity>()
                .Set(x => x.StoreTypeCode, StoreTypeCode.CommerceInterface)
                .Save();
            ...
            orderA = Create.Order<CommerceInterfaceOrderEntity>(store, context.Customer)
                .Set(x => x.CommerceInterfaceOrderNumber, "1000")
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "10")
                .Save();
            orderB = Create.Order<CommerceInterfaceOrderEntity>(store, context.Customer)
                .Set(x => x.CommerceInterfaceOrderNumber, "2000")
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "20")
                .Save();
            orderD = Create.Order<CommerceInterfaceOrderEntity>(store, context.Customer)
                .Set(x => x.CommerceInterfaceOrderNumber, "3000")
        ...
        /// </summary>
        private async Task<(CommerceInterfaceOrderEntity original, CommerceInterfaceOrderEntity split)> PerformSplit(OrderEntity order)
        {
            ...
            var original = await dataProvider.GetEntityAsync<CommerceInterfaceOrderEntity>(result.First());
            var split = await dataProvider.GetEntityAsync<CommerceInterfaceOrderEntity>(result.Last());
