...
namespace ShipWorks.Stores.Tests.Integration.Platforms.ClickCartPro
{
    ...
    public class ClickCartProCombineAndSplitTest : IDisposable
    {
        ...
        public ClickCartProCombineAndSplitTest(DatabaseFixture db, ITestOutputHelper output)
        {
            this.output = output;
            ...
            orderA = Create.Order<ClickCartProOrderEntity>(store, context.Customer)
                .Set(x => x.ClickCartProOrderID, "1000")
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "10")
                .Save();
            orderB = Create.Order<ClickCartProOrderEntity>(store, context.Customer)
                .Set(x => x.ClickCartProOrderID, "2000")
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "20")
                .Save();
            orderD = Create.Order<ClickCartProOrderEntity>(store, context.Customer)
                .Set(x => x.ClickCartProOrderID, "3000")
        ...
        public async Task Split_WithOrderNumbers()
        {
            ...
            var identityProvider = context.Mock.Container.Resolve<ClickCartProCombineOrderNumberCompleteSearchProvider>();
        ...
        /// </summary>
        private async Task<(ClickCartProOrderEntity original, ClickCartProOrderEntity split)> PerformSplit(OrderEntity order)
        {
            ...
            var original = await dataProvider.GetEntityAsync<ClickCartProOrderEntity>(result.First());
            var split = await dataProvider.GetEntityAsync<ClickCartProOrderEntity>(result.Last());
