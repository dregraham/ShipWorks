...
namespace ShipWorks.Stores.Tests.Integration.Platforms.SparkPay
{
    public partial class SparkPayOnlineUpdateCommandCreatorTest : IDisposable
    {
        ...
        public async Task UploadDetails_MakesOneWebRequest_WhenOrderIsNotCombined()
        {
            ...
            webClient.Verify(x => x.AddShipment(store, ShipmentWithTrackingNumber("track-123"), 10));
        }
        ...
        public async Task UploadDetails_MakesOneWebRequest_WhenOneOfTwoNonCombinedOrdersIsManual()
        {
            ...
            webClient.Verify(x => x.AddShipment(store, AnyShipment, 20), Times.Never);
            webClient.Verify(x => x.AddShipment(store, ShipmentWithTrackingNumber("track-123"), 10));
        }
        ...
        public async Task UploadDetails_MakesTwoWebRequests_WhenOrderIsCombined()
        {
            ...
            webClient.Verify(x => x.AddShipment(store, ShipmentWithTrackingNumber("track-123"), 20));
            webClient.Verify(x => x.AddShipment(store, ShipmentWithTrackingNumber("track-123"), 30));
        }
        ...
        public async Task UploadDetails_MakesThreeWebRequests_WhenBothCombinedAndNonCombinedAreUploaded()
        {
            ...
            webClient.Verify(x => x.AddShipment(store, ShipmentWithTrackingNumber("track-123"), 10));
            webClient.Verify(x => x.AddShipment(store, ShipmentWithTrackingNumber("track-456"), 50));
            webClient.Verify(x => x.AddShipment(store, ShipmentWithTrackingNumber("track-456"), 60));
        }
        ...
        public async Task UploadDetails_ContinuesUploading_WhenFailuresOccur()
        {
            ...
            webClient.Setup(x => x.AddShipment(store, AnyShipment, 10))
                .Throws(new SparkPayException("Foo"));
            webClient.Setup(x => x.AddShipment(store, AnyShipment, 50))
                .Throws(new SparkPayException("Foo"));
            ...
            webClient.Verify(x => x.AddShipment(store, ShipmentWithTrackingNumber("track-456"), 60));
            webClient.Verify(x => x.AddShipment(store, ShipmentWithTrackingNumber("track-789"), 70));
        }
    }
}