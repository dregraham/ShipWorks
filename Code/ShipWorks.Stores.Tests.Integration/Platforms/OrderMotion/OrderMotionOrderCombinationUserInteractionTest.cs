...
namespace ShipWorks.Stores.Tests.Integration.Platforms.OrderMotion
{
    ...
    public class OrderCombinationUserInteraction : IDisposable
    {
        ...
        public OrderCombinationUserInteraction(DatabaseFixture db, ITestOutputHelper output)
        {
            ...
            Create.Order<OrderMotionOrderEntity>(store, context.Customer).Save();

            var order1 = Create.Order<OrderMotionOrderEntity>(store, context.Customer)
                .Set(x => x.OrderMotionShipmentID, 1000)
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "100")
                .Save();
            var order2 = Create.Order<OrderMotionOrderEntity>(store, context.Customer)
                .Set(x => x.OrderMotionShipmentID, 2000)
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "200")
                .Save();
            var order3 = Create.Order<OrderMotionOrderEntity>(store, context.Customer)
                .Set(x => x.OrderMotionShipmentID, 3000)
        ...
        public async Task VariousOrderCombinations(bool[] manualOrders, int firstSurviving, int secondSurviving)
        {
            ...
            var storeQuery = factory.OrderMotionOrderSearch.Where(OrderMotionOrderSearchFields.OrderID == finalOrder.OrderID);
            ...
            using (ISqlAdapter sqlAdapter = context.Mock.Container.Resolve<ISqlAdapterFactory>().Create())
            {
                ...
                var orderTests = orders.Take(3).Select(x => x.Value)
                    ...
                    .LeftJoin(storeResults.OfType<IOrderMotionOrderSearchEntity>(), x => x.Item1.OrderID, x => x.OriginalOrderID)
                ...
                for (int i = 0; i < orderTests.Length; i++)
                {
                    ...
                    if (manualOrders[i])
                    {
                        Assert.Null(test.StoreSearch as IOrderMotionOrderSearchEntity);
                    }
                    else
                    {
                        Assert.Equal((test.Order as IOrderMotionOrderEntity).OrderMotionShipmentID,
                            (test.StoreSearch as IOrderMotionOrderSearchEntity).OrderMotionShipmentID);
                    }
                }
            }
        }
