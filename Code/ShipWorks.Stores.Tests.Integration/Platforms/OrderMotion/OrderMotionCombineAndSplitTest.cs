...
namespace ShipWorks.Stores.Tests.Integration.Platforms.OrderMotion.OnlineUpdating
{
    [Collection("Database collection")]
    [Trait("Category", "ContinuousIntegration")]
    [Trait("Category", "CombineSplit")]
    public class OrderMotionCombineAndSplitTest : IDisposable
    {
        ...
        public OrderMotionCombineAndSplitTest(DatabaseFixture db, ITestOutputHelper output)
        {
            this.output = output;
            ...
            orderA = Create.Order<OrderMotionOrderEntity>(store, context.Customer)
                .Set(x => x.OrderMotionShipmentID, 1000)
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "10")
                .Save();

            orderB = Create.Order<OrderMotionOrderEntity>(store, context.Customer)
                .Set(x => x.OrderMotionShipmentID, 2000)
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "20")
                .Save();

            orderD = Create.Order<OrderMotionOrderEntity>(store, context.Customer)
        ...
        public async Task Split_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 1000L }, identities_A_3.Select(i => i.OrderMotionShipmentID));
            Assert.Equal(new[] { 1000L }, identities_A_4.Select(i => i.OrderMotionShipmentID));
            Assert.Equal(new[] { 1000L }, identities_A_5.Select(i => i.OrderMotionShipmentID));
        }
        ...
        public async Task SplitCombine_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 1000L }, identities_A_C.Select(i => i.OrderMotionShipmentID));
        }
        ...
        public async Task SplitCombine_SplitSurvivingOrder_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 1000L }, identities_A_1_C.Select(i => i.OrderMotionShipmentID));
        }
        ...
        public async Task SplitACombineBCombineRemainingTwo_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 1000L, 2000L }, identities_A_1_C_1.Select(i => i.OrderMotionShipmentID));
        }
        ...
        public async Task SplitManualCombineWithNotManualOrder_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 2000L }, identities_A_M_1_C.Select(i => i.OrderMotionShipmentID));
        ...
        public async Task CombineMixManualSplit_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 2000L }, identities_B_0.Select(i => i.OrderMotionShipmentID));
            Assert.Equal(new[] { 2000L }, identities_B_1.Select(i => i.OrderMotionShipmentID));
        }
        ...
        public async Task SplitThenCombineOrder_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 1000L, 2000L }, identities_A_C.Select(i => i.OrderMotionShipmentID));
            Assert.Equal(new[] { 1000L }, identities_A_1.Select(i => i.OrderMotionShipmentID));
        }
        ...
        public async Task CombineSplitWithBSurviving_WithOrderNumbers()
        {
            ...
            Assert.Equal(new[] { 1000L, 2000L }, identities_B_0.Select(i => i.OrderMotionShipmentID));
            Assert.Equal(new[] { 1000L, 2000L }, identities_B_1.Select(i => i.OrderMotionShipmentID));
        }
        ...
        /// </summary>
        private async Task<(OrderMotionOrderEntity original, OrderMotionOrderEntity split)> PerformSplit(OrderEntity order)
        {
            ...
            var original = await dataProvider.GetEntityAsync<OrderMotionOrderEntity>(result.First());
            var split = await dataProvider.GetEntityAsync<OrderMotionOrderEntity>(result.Last());
