...
namespace ShipWorks.Stores.Tests.Integration.Platforms.Groupon
{
    ...
    public class OrderCombinationUserInteraction : IDisposable
    {
        ...
        public OrderCombinationUserInteraction(DatabaseFixture db, ITestOutputHelper output)
        {
            ...
            Create.Order<GrouponOrderEntity>(store, context.Customer).Save();

            var order1 = Create.Order<GrouponOrderEntity>(store, context.Customer)
                .Set(x => x.GrouponOrderID, "1000")
                .Set(x => x.ParentOrderID, "1000000")
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "100")
                .Save();
            var order2 = Create.Order<GrouponOrderEntity>(store, context.Customer)
                .Set(x => x.GrouponOrderID, "2000")
                .Set(x => x.ParentOrderID, "2000000")
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "200")
                .Save();
            var order3 = Create.Order<GrouponOrderEntity>(store, context.Customer)
                .Set(x => x.GrouponOrderID, "3000")
                .Set(x => x.ParentOrderID, "3000000")
        ...
        public async Task VariousOrderCombinations(bool[] manualOrders, int firstSurviving, int secondSurviving)
        {
            ...
            var storeQuery = factory.GrouponOrderSearch.Where(GrouponOrderSearchFields.OrderID == finalOrder.OrderID);
            ...
            using (ISqlAdapter sqlAdapter = context.Mock.Container.Resolve<ISqlAdapterFactory>().Create())
            {
                ...
                var orderTests = orders.Take(3).Select(x => x.Value)
                    ...
                    .LeftJoin(storeResults.OfType<IGrouponOrderSearchEntity>(), x => x.Item1.OrderID, x => x.OriginalOrderID)
                ...
                for (int i = 0; i < orderTests.Length; i++)
                {
                    ...
                    if (manualOrders[i])
                    {
                        Assert.Null(test.StoreSearch as IGrouponOrderSearchEntity);
                    }
                    else
                    {
                        Assert.Equal((test.Order as IGrouponOrderEntity).GrouponOrderID,
                            (test.StoreSearch as IGrouponOrderSearchEntity).GrouponOrderID);
                        Assert.Equal((test.Order as IGrouponOrderEntity).ParentOrderID,
                            (test.StoreSearch as IGrouponOrderSearchEntity).ParentOrderID);
                    }
                }
            }
        }
