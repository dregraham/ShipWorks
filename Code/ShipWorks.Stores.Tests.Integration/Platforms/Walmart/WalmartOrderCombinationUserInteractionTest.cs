...
namespace ShipWorks.Stores.Tests.Integration.Platforms.Walmart
{
    ...
    public class WalmartOrderCombinationUserInteraction : IDisposable
    {
        ...
        public WalmartOrderCombinationUserInteraction(DatabaseFixture db, ITestOutputHelper output)
        {
            this.output = output;
            ...
            store = Create.Store<WalmartStoreEntity>(StoreTypeCode.Walmart)
                .Save();

            // Create a dummy order that serves as a guarantee that we're not just fetching all orders later
            Create.Order<WalmartOrderEntity>(store, context.Customer).Save();

            var order1 = Create.Order<WalmartOrderEntity>(store, context.Customer)
                .Set(x => x.PurchaseOrderID, "1000")
                .Set(x => x.OrderNumber, 10)
                .Set(x => x.OrderNumberComplete, "100")
                .Save();
            var order2 = Create.Order<WalmartOrderEntity>(store, context.Customer)
                .Set(x => x.PurchaseOrderID, "2000")
                .Set(x => x.OrderNumber, 20)
                .Set(x => x.OrderNumberComplete, "200")
                .Save();
            var order3 = Create.Order<WalmartOrderEntity>(store, context.Customer)
                .Set(x => x.PurchaseOrderID, "3000")
        ...
        public async Task VariousOrderCombinations(bool[] manualOrders, int firstSurviving, int secondSurviving)
        {
            ...
            var storeQuery = factory.WalmartOrderSearch.Where(WalmartOrderSearchFields.OrderID == finalOrder.OrderID);
            ...
            using (ISqlAdapter sqlAdapter = context.Mock.Container.Resolve<ISqlAdapterFactory>().Create())
            {
                ...
                var orderTests = orders.Take(3).Select(x => x.Value)
                    ...
                    .LeftJoin(storeResults.OfType<IWalmartOrderSearchEntity>(), x => x.Item1.OrderID, x => x.OriginalOrderID)
                ...
                for (int i = 0; i < orderTests.Length; i++)
                {
                    ...
                    if (manualOrders[i])
                    {
                        Assert.Null(test.StoreSearch as IWalmartOrderSearchEntity);
                    }
                    else
                    {
                        Assert.Equal((test.Order as IWalmartOrderEntity).PurchaseOrderID,
                            (test.StoreSearch as IWalmartOrderSearchEntity).PurchaseOrderID);
                    }
                }
            }
        }
