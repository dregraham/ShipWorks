...
namespace ShipWorks.Stores.Tests.Integration.Platforms.BuyDotCom
{
    ...
    public class BuyDotComDataAccessTest : IDisposable
    {
        private readonly DataContext context;
        private readonly BuyDotComStoreEntity store;
        private readonly OrderEntity order;
        private readonly ShipmentEntity shipment;
        private readonly BuyDotComDataAccess testObject;

        public BuyDotComDataAccessTest(DatabaseFixture db)
        {
            context = db.CreateDataContext(x => ContainerInitializer.Initialize(x));

            testObject = context.Mock.Create<BuyDotComDataAccess>();
        ...
        [Fact]
        public async Task GetShipmentDataByOrderAsync_ReturnsDetails_WhenOrderExists()
        {
            var shipmentDetails = await testObject.GetShipmentDataByOrderAsync(new[] { order.OrderID });
        ...
        [Fact]
        public async Task GetShipmentDataByOrderAsync_ReturnsCombinedDetails_WhenOrderIsCombined()
        {
            Modify.Order(order)
        ...
        [Fact]
        public async Task GetShipmentDataByOrderAsync_ExtraShipmentDataIsLoaded()
        {
        ...
        [Fact]
        public async Task GetShipmentDataByOrderAsync_ReturnsEmptyDetails_WhenOrderHasNoShipments()
        {
        ...
        [Fact]
        public async Task GetShipmentDataByOrderAsync_ReturnsEmptyDetails_WhenOrderIsNotFound()
        {
            var shipmentDetails = await testObject.GetShipmentDataByOrderAsync(new[] { -1006L });

            Assert.Empty(shipmentDetails);
        }

        [Fact]
        public async Task GetShipmentDataByShipmentAsync_ReturnsDetails_WhenOrderExists()
        {
            var shipmentDetails = await testObject.GetShipmentDataByShipmentAsync(new[] { shipment.ShipmentID });
        ...
        [Fact]
        public async Task GetShipmentDataByShipmentAsync_ReturnsCombinedDetails_WhenOrderIsCombined()
        {
            Modify.Order(order)
                .WithItem<BuyDotComOrderItemEntity>(i => i.Set(x => x.OriginalOrderID, 4006))
                .WithItem<BuyDotComOrderItemEntity>(i => i.Set(x => x.OriginalOrderID, 4006))
        ...
        [Fact]
        public async Task GetShipmentDataByShipmentAsync_ExtraShipmentDataIsLoaded()
        {
            Modify.Shipment(shipment)
                .AsPostal(p => p.AsUsps(u => u.Set(x => x.UspsAccountID, 123)))
                .Save();
        ...
        [Fact]
        public async Task GetShipmentDataByShipmentAsync_ReturnsEmptyDetails_WhenOrderIsNotFound()
        {
            var shipmentDetails = await testObject.GetShipmentDataByShipmentAsync(new[] { -1031L });
