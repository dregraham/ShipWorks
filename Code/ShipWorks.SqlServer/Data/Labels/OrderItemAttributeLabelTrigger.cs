...
public partial class Triggers
{
    ...
    public static void OrderItemAttributeLabelTrigger()
    {
        using (SqlConnection con = new SqlConnection("Context connection = true"))
        {
            ...
            if (SqlContext.TriggerContext.TriggerAction == TriggerAction.Update)
            {
                ...
                cmd.CommandText =
                    @"UPDATE ObjectLabel
                      SET Label = LEFT(u.Name, 100)
                      FROM inserted u
                      WHERE ObjectLabel.ObjectID = u.OrderItemAttributeID";
                cmd.ExecuteNonQuery();
            }
            ...
            if (SqlContext.TriggerContext.TriggerAction == TriggerAction.Delete)
            {
                ...
                UPDATE ObjectLabel
                  SET IsDeleted = 1
                  FROM deleted d
                  WHERE ObjectLabel.ObjectID = d.OrderItemAttributeID";
                cmd.ExecuteNonQuery();
            }
        }
    }
