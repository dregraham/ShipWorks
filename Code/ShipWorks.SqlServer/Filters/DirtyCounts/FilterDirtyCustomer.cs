    ...
    public static void FilterDirtyCustomer()
    {
        ...
        using (SqlConnection con = new SqlConnection("Context connection = true"))
        {
            ...
            if (SqlContext.TriggerContext.TriggerAction == TriggerAction.Delete)
            {
                ...
                actionCmd.CommandText = @"
                    INSERT INTO ActionQueue(ActionID, ActionName, TriggerComputerID, ComputerLimitedList, ObjectID, Status, NextStep)
                       SELECT t.ActionID, '', @computerID, CASE t.ComputerLimitedType WHEN 0 THEN '' WHEN 1 THEN CAST(@computerID as varchar(150)) ELSE t.ComputerLimitedList END, d.CustomerID, 0, 0
            ...
            else if (SqlContext.TriggerContext.TriggerAction == TriggerAction.Insert)
            {
                ...
                actionCmd.CommandText = @"
                    INSERT INTO ActionQueue(ActionID, ActionName, TriggerComputerID, ComputerLimitedList, ObjectID, Status, NextStep)
                       SELECT t.ActionID, '', @computerID, CASE t.ComputerLimitedType WHEN 0 THEN '' WHEN 1 THEN CAST(@computerID as varchar(150)) ELSE t.ComputerLimitedList END, i.CustomerID, 0, 0
