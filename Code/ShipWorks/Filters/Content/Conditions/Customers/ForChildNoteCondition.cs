...
namespace ShipWorks.Filters.Content.Conditions.Customers
{
    ...
    public abstract class ForChildNoteCondition : ContainerCondition
    {
        ...
        public static string GetChildPredicate(SqlGenerationContext context)
        {
            ...
            // This is the sql to match the notes for the customer. For the Customer note table alias, we use a placeholder {0}, which will
            ...
            // This is the sql to match the notes for the child orders. For the Order Note table alias, we use a placeholder {0}, which will
            // get replaces by the scope once the alias is created.
            ...
            string childPredicate = string.Format("(({0}) OR ({1}))", customerNotes, orderNotes);
