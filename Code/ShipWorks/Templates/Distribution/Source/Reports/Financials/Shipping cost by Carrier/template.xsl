...
<xsl:stylesheet version="1.0" 
    ...
    <body style="{$pageFont}">
        ...
        <h3>Shipping Cost by Carrier - <xsl:value-of select="count(//Order)" /> Orders</h3>
        ...
        <table style="width:4in; margin: 0px 0px -6px 0px; border-collapse: collapse;" cellspacing="0">
            ...
            <tr>
                ...
                <td style="{$rowStyle};" align="right">$<xsl:value-of select="format-number($other, '#,##0.00')" /></td>
            </tr>

            <tr>
                <td style="{$rowStyle};">FedEx</td>
                <td style="{$rowStyle};" align="right">$<xsl:value-of select="format-number($fedex, '#,##0.00')" /></td>
            </tr>

            <tr>
                <td style="{$rowStyle};">UPS (Integrated)</td>
                <td style="{$rowStyle};" align="right">$<xsl:value-of select="format-number($ups, '#,##0.00')" /></td>
            </tr>

            <tr>
                <td style="{$rowStyle};">UPS (WorldShip)</td>
                <td style="{$rowStyle};" align="right">$<xsl:value-of select="format-number($upsWorldShip, '#,##0.00')" /></td>
            </tr>

            <tr>
                <td style="{$rowStyle};">USPS (w/o Postage)</td>
                <td style="{$rowStyle};" align="right">$<xsl:value-of select="format-number($uspsWeb, '#,##0.00')" /></td>
            </tr>

            <tr>
                <td style="{$rowStyle};">USPS (Endicia)</td>
                <td style="{$rowStyle};" align="right">$<xsl:value-of select="format-number($uspsEndicia, '#,##0.00')" /></td>
            </tr>
            
            <tr>
                <td style="{$rowStyle};">USPS (Express1)</td>
                <td style="{$rowStyle};" align="right">$<xsl:value-of select="format-number($uspsExpress1, '#,##0.00')" /></td>
            </tr>            

            <tr>
                <td style="{$rowStyle};">USPS (Stamps.com)</td>
                <td style="{$rowStyle};" align="right">$<xsl:value-of select="format-number($uspsStamps, '#,##0.00')" /></td>
            </tr>
            
            <tr>
                <td style="{$rowStyle};"><b>Total:</b></td>
