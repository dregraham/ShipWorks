...
namespace ShipWorks.Stores.Communication
{
    ...
    public abstract class StoreDownloader
    {
        ...
        private readonly string orderStatusText = string.Empty;
        private readonly string itemStatusText = string.Empty;
        private readonly bool orderStatusHasTokens = false;
        private readonly bool itemStatusHasTokens = false;
        ...
        protected StoreDownloader([Obfuscation(Exclude = true)] StoreEntity store, StoreType storeType)
        {
            ...
            // TODO: StatusPresetManager needs to be injected otherwise tests will fail.
...
// TODO:  This Wait() should probably be moved outside the transaction!!!
        ...
        private void SaveNewOrder(OrderEntity order, SqlAdapter adapter)
        {
            ...
            Task<CustomerEntity> getCustomerTask = Task.Run(() =>
            {
                try
                {
                    return CustomerProvider.AcquireCustomer(order, storeType, adapter, false);
                }
            ...
            // Wait for the cusotmer to be found or created
