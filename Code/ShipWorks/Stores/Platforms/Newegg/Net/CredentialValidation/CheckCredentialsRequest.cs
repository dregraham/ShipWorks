...
namespace ShipWorks.Stores.Platforms.Newegg.Net.CredentialValidation
{
    ...
    public class CheckCredentialsRequest : ICheckCredentialRequest
    {
        ...
        {
            NeweggResponse response = await SubmitRequest(credentials).ConfigureAwait(false);
            return response.ResponseErrors.None();
        }

        /// <summary>
        /// Submit the request
        /// </summary>
        private async Task<NeweggResponse> SubmitRequest(Credentials credentials)
        {
            ...
            string rawResponseData = await request.SubmitRequest(credentials, requestConfig).ConfigureAwait(false);
