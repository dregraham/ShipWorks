...
|
| Copyright 2017 Interapptive, Inc.  All rights reserved.
...
$openCartVersion = GetOpenCartVersion();
...
foreach ($query->rows as $setting) {
	...
	{
		$config->set($setting['key'], VersionSpecificDecode($setting['value'], $openCartVersion));
	}
}

if (version_compare($openCartVersion, '2.2') >= 0)
{
	// Event
...
{
	$versionPattern = '/define\\(\'VERSION\', \'(\\d*\\.\\d*\\.\\d*\\.\\d*)\'\\);/i';

	foreach (file('index.php') as $line)
	{
		$matches = array();
		if (preg_match($versionPattern, $line, $matches))
		{
			return $matches[1];
		}
	}
...
function VersionSpecificDecode($value, $version) {
	return version_compare($version, '2.1') >= 0 ?
		json_decode($value) :
		unserialize($value);
}
