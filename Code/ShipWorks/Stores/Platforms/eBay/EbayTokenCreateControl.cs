...
namespace ShipWorks.Stores.Platforms.Ebay
{
    ...
    public partial class EbayTokenCreateControl : UserControl
    {
        ...
        {
            InitializeComponent();
            authorizationToken = new Authorization.Token(ShipWorksSession.InstanceID.ToString());
        }
        ...
        private void OnTimerTick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    if (store.EBayUserID.Length > 0 && store.EBayUserID != authorizationToken.UserId)
                    {
                        throw new EbayException("You cannot switch to a different eBay user in the free edition of ShipWorks.");
                    }
                }

                store.EBayToken = authorizationToken.Key;
                store.EBayUserID = authorizationToken.UserId;
                store.EBayTokenExpire = authorizationToken.ExpirationDate;
