    ...
    public class EbayCombinedOrderCandidate
    {
        ...
        private async Task<bool> CombineLocalOrders(ISqlAdapter adapter, List<EbayCombinedOrderComponent> toCombine, long? ebayOrderID)
        {
            ...
            foreach (EbayCombinedOrderComponent component in toCombine)
            {
                ...
                if (locallyCombinedOnly)
                {
                    ...
                    else if (component.Order.OrderItems.Count > 0 && combinedOrderRelations.All(e => e.EbayOrderID != component.Order.EbayOrderID))
                    {
        ...
        private void MoveCharges(EbayOrderEntity source, EbayOrderEntity target)
        {
            ...
            foreach (OrderChargeEntity sourceCharge in source.OrderCharges.ToList())
            {
                ...
                if (target.OrderItems.Count > 0 && new List<string> { "TAX", "SHIPPING", "ADJUST", "OTHER" }.Contains(sourceCharge.Type))
                {
                    continue;
                }
