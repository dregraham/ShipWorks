...
namespace ShipWorks.Stores.Platforms.Ebay.OrderCombining
{
    ...
    {
        // Logger 
        static readonly ILog log = LogManager.GetLogger(typeof(CombinedOrder));

        /// <summary>
        /// Orders that may or may not be included in the final combined order
        /// </summary>
        List<CombinedOrderComponent> components = new List<CombinedOrderComponent>();
        public IList<CombinedOrderComponent> Components
        {
        ...
        /// </summary>
        public CombinedOrder(long storeID, CombineType combineType, string ebayBuyerID, List<EbayOrderEntity> orders)
        {
            if (orders.Count == 0)
            {
            ...
            orders.ForEach(o => components.Add(new CombinedOrderComponent(o, true)));
        }
        ...
        public void DiscoverRelatedOrders()
        {
            using (SqlAdapter adapter = new SqlAdapter())
            {
                ...
                RelationPredicateBucket bucket = new RelationPredicateBucket(EbayOrderFields.StoreID == order.StoreID
                    & EbayOrderFields.EbayBuyerID == EbayBuyerID & EbayOrderFields.EbayOrderID == 0 & OrderFields.OnlineLastModified >= DateTime.UtcNow.Subtract(TimeSpan.FromDays(14)));

                // Get order items, shipments, etc.
                ...
                foreach (EbayOrderEntity foundOrder in orders)
                {
                    ...
                    {
                        bool add = (CombineType == CombineType.Local) ? EbayUtility.GetEffectiveCheckoutStatus((EbayOrderItemEntity)foundOrder.OrderItems.First()) == EbayEffectiveCheckoutStatus.Paid :
                        ...
                        if (add)
                        {
                            // add it to the this combined order, but it won't be selected by default since 
                            // the user didn't select manually
                            components.Add(new CombinedOrderComponent(foundOrder, false));
                        }
                    }
                }
            }
        ...
        public bool Combine()
        {
            List<CombinedOrderComponent> toCombine = components.Where(c => c.Included).ToList();
            ...
            if (CombineType == CombineType.EbayCombinedPayment)
            {
                ...
                foreach (CombinedOrderComponent c in toCombine)
                {
                    foreach (EbayOrderItemEntity orderItem in c.Order.OrderItems)
                    {
                        ...
                        TransactionType trans = new TransactionType()
                        {
                            ...
                            Item = new ItemType()
                            {
                                ItemID = orderItem.EbayItemID.ToString()
                            }
                        };
        ...
        /// </summary>
        private double GetCombinedPaymentTotal(List<CombinedOrderComponent> toCombine)
        {
            decimal total = 0;
        ...
        /// is related to an eBay Combined Payment (what ebay calls an Order), ebayOrderID will be non-null
        /// </summary>
        private bool CombineLocalOrders(List<CombinedOrderComponent> toCombine, long? ebayOrderID)
        {
            ...
            if (storeEntity == null)
            {
                log.WarnFormat("Unable to combine orders for buyer {0}, store has been deleted.", EbayBuyerID);
                return false;
            }
            ...
            foreach (CombinedOrderComponent component in toCombine)
            {
