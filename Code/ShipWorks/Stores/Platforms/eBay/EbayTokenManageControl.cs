    ...
    public partial class EbayTokenManageControl : UserControl
    {
        ...
        private void OnExportTokenFile(object sender, EventArgs e)
        {
            if (store.EBayToken.Length > 0)
            {
                ...
                using (TokenExportDialog exportDialog = new TokenExportDialog())
                {
                    if (exportDialog.Show(this) == DialogResult.OK)
                    {
                        try
                        {
                            ...
                            TokenData tokenData = new TokenData { ExpirationDate = store.EBayTokenExpire, Key = store.EBayToken, UserId = store.EBayUserID };
                            Token token = new Token(ShipWorksSession.InstanceID.ToString(), tokenData);
