...
namespace ShipWorks.Stores.Platforms.Ebay.Tokens.Writers
{
    ...
    public class XmlTokenWriter : ITokenWriter, IDisposable
    {
        ...
        public virtual void Write(EbayToken token)
        {
        ...
        /// </summary>
        /// <returns></returns>
        protected string BuildTokenXml(EbayToken token)
        {
...
<eBayToken>
  ...
  <UserID>{2}</UserID>
            ...
            return string.Format(TokenXmlFormat, token.Token, token.ExpirationDate.ToString("yyyy-MM-dd HH:mm:ss"), token.UserId);
        }
