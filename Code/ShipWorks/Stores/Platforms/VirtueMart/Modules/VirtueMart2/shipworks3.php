<?php
	...
	function Action_GetStore()
	{
		...
		$sql = "SELECT * FROM ". $dbPrefix. "virtuemart_vmusers vmu" .
				...
				" inner join ". $dbPrefix. "virtuemart_userinfos ui on ui.virtuemart_user_id = vmu.virtuemart_user_id" .
				" inner join ". $dbPrefix. "virtuemart_vendors_en_gb en on en.virtuemart_vendor_id = vmu.virtuemart_vendor_id" .
				" inner join ". $dbPrefix. "virtuemart_states s on s.virtuemart_state_id = ui.virtuemart_state_id".
				" inner join ". $dbPrefix. "virtuemart_countries c on c.virtuemart_country_id = ui.virtuemart_country_id".	
	...
	function WriteOrder($row)
	{
		...
		writeElement("LastModified", toFormattedDateFromString($row['modified_on']));
		writeElement("ShippingMethod", GetShippingMethod($row['virtuemart_shipmentmethod_id']));
		...
		$sqlAddressWithoutType = "select u.*, s.state_name, c.country_name from ". $dbPrefix. "virtuemart_order_userinfos u".
				" left outer join ". $dbPrefix. "virtuemart_states s on s.virtuemart_state_id = u.virtuemart_state_id".
				" inner join ". $dbPrefix. "virtuemart_countries c on c.virtuemart_country_id = u.virtuemart_country_id".
				" where virtuemart_order_id = " . $row['virtuemart_order_id'] . " AND address_type=";
		...
		writeElement("Method", GetPaymentMethod($row['virtuemart_paymentmethod_id']));
	...
	function GetShippingMethod($shipping_method_id)
	{
		...
		$sql = "SELECT * FROM ". $dbPrefix. "virtuemart_shipmentmethods_en_gb sm" .
	...
	function WriteOrderTotals($order)
	{
		...
		{
            WriteTotal($i++, 'Shipping Tax', $order['order_shipment_tax'], 'shipping tax');
		}
	...
	function WriteOrderItems($orderID)
	{
		...
		while($item = mysql_fetch_array($itemQuery))
		{
			...
			$sql = "select p.*,m.file_url_thumb". 
				...
				" inner join ". $dbPrefix. "virtuemart_product_medias pm on pm.virtuemart_product_id = p.virtuemart_product_id".
				" inner join ". $dbPrefix. "virtuemart_medias m on m.virtuemart_media_id = pm.virtuemart_media_id".
			...
			// Write attributes
			WriteItemAttributes($item['virtuemart_product_id'], $item['product_attribute']);
	...
	function Action_GetStatusCodes()
	{
		...
		$codesQuery = mysql_query("SELECT * FROM ". $dbPrefix. "virtuemart_orderstates");
