...
namespace ShipWorks.Stores.Platforms.ThreeDCart.AdvancedApi
{
    ...
    /// 
    /// This class is used to return the appropriate sql statements for MS Access.
    /// </summary>
    public class MSAccessQueryProvider : IApiQueryProvider
    {
        /// <summary>
        ...
        /// </summary>
        /// <value>SQL query that should NOT return an error when run against an MS Access Database.</value>
        public string ValidSqlStatementForThisDatabaseTypeOnly
        {
            get
            {
                return "select iif(true, true, false)";
            }
        }
        ...
        {
            return string.Format("select top {0} orderid, invoicenum, invoicenum_prefix, (odate + otime) as odate, last_update from [Orders] " +
