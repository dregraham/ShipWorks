...
namespace ShipWorks.Editions.Freemium
{
    ...
    public class FreemiumFreeEdition : Edition
    {
            ...
            : base(store)
        {
            ...
            {
                if (shipmentType == ShipmentTypeCode.Endicia || shipmentType == ShipmentTypeCode.PostalExpress1)
                {
                    continue;
                }

                AddRestriction(EditionFeature.ShipmentType, shipmentType, (shipmentType == ShipmentTypeCode.Stamps) ? EditionRestrictionLevel.Hidden : EditionRestrictionLevel.RequiresUpgrade);
            }
